    ...
    if uploaded_file is not None:
        try:
            ...
            if raw_df.shape[1] != 7:
                ...
                # Convert numeric columns to proper numeric types to ensure totals calculate correctly
                result_df_for_pivot["DEBIT"] = pd.to_numeric(result_df_for_pivot["DEBIT"], errors='coerce')
                result_df_for_pivot["CREDIT"] = pd.to_numeric(result_df_for_pivot["CREDIT"], errors='coerce')
                
                # Replace NaN in CPT with "Vide"
                ...
                with pd.ExcelWriter(output, engine="openpyxl") as writer:
                    ...
                    # Find the row indices where CPT is "Vide" to apply red highlight
                    # and also format total rows with light gray background
                    for row_idx, row in enumerate(pivot_sheet.iter_rows(min_row=2, max_row=pivot_sheet.max_row), 2):
                        # Get the value from cell A (CPT column)
                        cpt_value = row[0].value
                        
                        # Check if this is a "Vide" row
                        if cpt_value == "Vide":
                            # Apply light red fill to all cells in this row
                            for cell in row:
                                cell.fill = light_red_fill
                        
                        # Check if this is a total row (either CPT total or grand total)
                        elif cpt_value == "Total" or (row[1].value == "Total" and cpt_value is not None):
                            # Apply light gray fill and bold font to all cells in this row
                            for cell in row:
